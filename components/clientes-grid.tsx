...
export function ClienteCardUI({ cliente, onClick }: ClienteCardProps) {
  ...
  const CardContent = (
    ...
    >
      ...
      <div className="p-6 flex flex-col items-center text-center space-y-3">
        <div
          className={cn(
            "relative w-24 h-24 rounded-2xl overflow-hidden border-2 shadow-inner flex items-center justify-center",
            token.bgLight,
            token.border,
          )}
        >
  ...
  const hrefCliente = cliente.id
    ? `/dashboard/clientes/${cliente.id}`
    : cliente.href;
  return hrefCliente ? (
    <Link href={hrefCliente}>{CardContent}</Link>
  ) : (
    CardContent
  );
}
