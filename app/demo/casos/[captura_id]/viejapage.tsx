...
export default async function CasoPage({
  ...
  // Obtener todos los casos del mismo cliente para el sidebar
  const { data: casosMismoCliente } = await supabaseServer
    .from("vista_dashboard_riesgos_api")
    .select("captura_id, escenario, nivel_riesgo, puntaje_global, fecha_deteccion, vertical")
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 via-slate-100 to-slate-200">
      {/* SIDEBAR */}
      <RiskSidebar
        casos={casosMismoCliente || []}
        currentCapturaId={captura_id}
        clienteNombre={caso.nombre_cliente}
        clienteLogo={caso.logo_url}
      />

      {/* CONTENIDO PRINCIPAL */}
      <div className="max-w-7xl mx-auto">
        {/* BARRA SUPERIOR MINIMALISTA */}
        <div className="bg-white/80 backdrop-blur-md border-b border-slate-200/50 sticky top-0 z-30 shadow-sm">
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="text-right">
                ...
                <p className="text-xs font-bold text-slate-700">
                  {formatDate(caso.fecha_deteccion)}
                </p>
              </div>
            </div>
          </div>
